...
class CsmarService(object):
    ...
    def __init__(self):
        ...
        self.base_dir = os.path.expanduser("~/csmardata")
        self.zip_dir = os.path.join(self.base_dir, "zip")
    ...
    def getPackResult(self,columns,condition,tableName,*_other_args):
        '''
                    ...
                    返回打包文件的下载地址filePath，函数会自动下载到默认文件夹~/csmardata/zip/下面
        ...
        while True:
            ...
            if packResult['data']['status']=='1':
                ...
                data = r.data
                downloadDir = self.zip_dir
                os.makedirs(downloadDir, exist_ok=True)
                zipPath = os.path.join(downloadDir, signCode + ".zip")
    ...
    def unzipSingle(self,fileName):
        '''
                    函数功能：解压单个文件到默认文件夹~/csmardata/
                    
                    参数：
        
           fileName(str)：完整文件名称fileName。例如~/csmardata/zip/662348367897071616.zip
           
                    返回值：无
        '''
        #解压单个文件到默认文件夹~/csmardata/
        zf = zipfile.ZipFile(fileName)
        (filePath,tempFileName)=os.path.split(fileName)
        (preFileName,extendsName)=os.path.splitext(tempFileName)
        destDir = os.path.join(self.base_dir, preFileName)
    ...
    def loadData(self,fileName,count=10000):
        '''
                    函数功能：解析的csv文件，把结果存入dataList。例如~/csmardata/BETA_Ybeta.csv
                    
                    参数：
        
           fileName(str)：完整文件名称fileName。例如~/csmardata/BETA_Ybeta.csv
        ...
        if type(fileName).__name__!='str' or type(count).__name__!='int':
            self.logger.error('Parameter type error')
            return False
        with open(fileName, encoding='utf-8-sig', errors="ignore") as csvf:
